#!/usr/bin/env python3
"""
Monte Carlo Localization (MCL) node for Puzzlebot in Gazebo.

Implements Activity 5 from semana3 presentation, steps A–I:

  A. Gazebo simulator    → provides /odom (nav_msgs/Odometry) and
                           /scan  (sensor_msgs/LaserScan) via plugins.
  B. Known map           → loaded from maps/room_map.png (PNG occupancy image).
  C. Grid resolution     → MAP_RESOLUTION = 0.01 m / pixel.
  D. Sample particles    → N_PARTICLES random (x, y, θ) over free space.
  E. Score particles     → pixel-sum at beam endpoints on the map.
  F. Filter particles    → weighted resampling (keep high-score particles).
  G. Dead reckoning      → compute robot displacement from /odom delta.
  H. Move particles      → propagate each particle by the same displacement
                           (applied in the particle's own heading frame).
  I. Loop back to D      → triggered on every /scan message (~10 Hz).
