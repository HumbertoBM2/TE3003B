#!/usr/bin/env python3
"""
Detector ArUco propio usando OpenCV DICT_4X4_50.

aruco_ros 5.0.4 usa la librería aruco (Muñoz-Salinas) con su propio formato
de diccionario, que es INCOMPATIBLE con los marcadores OpenCV DICT_4X4_50
del lab. Este nodo reemplaza aruco_ros con detección OpenCV directa.

Suscribe:
  /video_source/raw          (sensor_msgs/Image)
  /video_source/camera_info  (sensor_msgs/CameraInfo)

Publica:
  /marker_publisher/markers  (aruco_msgs/MarkerArray)  ← mismo topic que usaba aruco_ros
"""
...
class ArucoDetector(Node):
    ...
    def __init__(self):
        ...
        # Hardcoded calibration (640x480) — updated if camera_info arrives
        ...
        # Probamos los diccionarios más comunes en orden.
        # El detector auto-selecciona el primero que detecte un marcador y queda fijo.
        ...
        self._active_dict_name = None  # se fija cuando detectamos por primera vez
        self._active_dict = None
        self._frames_checked = 0

        # camera_node publica con BEST_EFFORT — debemos coincidir
        ...
        # Log de estado cada 5 segundos mientras no detecte nada
    ...
    def _image_cb(self, msg):
        ...
        if ids is not None and len(ids) > 0:
            ...
            for mid, rvec, tvec in zip(ids.flatten(), rvecs, tvecs):
                ...
                m.header = ma.header
                m.id = int(mid)
                # OpenCV frame: x=derecha, y=abajo, z=adelante
                # EKF-SLAM lee: dx = pose.position.z (adelante), dy = -pose.position.x
