...
def generate_launch_description():
    ...
    arg_camera = DeclareLaunchArgument(
        ...
        description='Habilitar cámara, ArUco y QR reader')
    arg_map = DeclareLaunchArgument(
        'map_path', default_value=os.path.expanduser('~/maps/current'),
        description='Ruta base del mapa sin extensión')
    ...
    tf_laser = Node(package='tf2_ros', executable='static_transform_publisher',
        name='tf_lidar_laser',
        arguments=['0','0','0','0','0','0','lidar_link','laser'])
    tf_camera = Node(package='tf2_ros', executable='static_transform_publisher',
        name='tf_base_camera',
        arguments=['0.09','0','0.07','0','0','0','base_link','camera_link'],
        condition=IfCondition(camera_en))
    tf_cam_opt = Node(package='tf2_ros', executable='static_transform_publisher',
        name='tf_camera_optical',
        arguments=['0','0','0','-1.5707963','0','-1.5707963',
                   'camera_link','camera_optical_frame'],
        condition=IfCondition(camera_en))

    cmd_vel_relay = Node(package='puzzlebot_ros', executable='cmd_vel_to_wheels',
        name='cmd_vel_relay', output='screen')
    odometry = Node(package='puzzlebot_ros', executable='odometry_node',
        name='odometry_node',
        parameters=[{'wheel_radius': 0.05, 'wheel_separation': 0.174,
                     'enc_deadband': 0.10}],
        output='screen')
    lifter = Node(package='puzzlebot_ros', executable='lift_controller',
        name='lift_controller', output='screen')

  
    nav_and_mission = TimerAction(period=3.0, actions=[
        Node(
            package='sllidar_ros2', executable='sllidar_node',
            name='sllidar_node',
            parameters=[{'channel_type': 'serial', 'serial_port': '/dev/lidar',
        ...
        Node(package='puzzlebot_ros', executable='scan_restamper',
            name='scan_restamper',
            parameters=[{'input_topic': '/scan', 'target_frame': 'lidar_link',
                         'invert_angles': False, 'angle_offset_rad': 3.14159265}],
            output='screen'),
        Node(package='puzzlebot_ros', executable='map_server_node',
            name='map_server_node',
            parameters=[{'map_path': LaunchConfiguration('map_path')}],
            output='screen'),
        Node(package='puzzlebot_ros', executable='particle_localization',
            name='particle_localization', output='screen'),
        Node(package='puzzlebot_ros', executable='astar_navigator',
            name='astar_navigator', output='screen'),
   
        Node(package='puzzlebot_ros', executable='mission_node',
            name='mission_node', output='screen'),
    ])


    vision_nodes = TimerAction(period=8.0, actions=[
        Node(package='puzzlebot_ros', executable='camera_node',
            name='camera_node', output='screen',
            condition=IfCondition(camera_en)),
        Node(package='puzzlebot_ros', executable='aruco_node',
            name='aruco_node',
            parameters=[{
            ...
            output='screen', condition=IfCondition(camera_en)),
        Node(package='puzzlebot_ros', executable='aruco_map_odom',
            name='aruco_map_odom',
            parameters=[{
            ...
            output='screen', condition=IfCondition(camera_en)),
        Node(package='puzzlebot_ros', executable='qr_reader_node',
            name='qr_reader_node',
            parameters=[{'detect_hz': 5.0, 'publish_image': True,
                         'min_qr_area_px': 150}],
            output='screen', condition=IfCondition(camera_en)),
    ])

    return LaunchDescription([
        unset_fastdds, arg_camera, arg_map,
        micro_ros,
        tf_base, tf_lidar, tf_laser, tf_camera, tf_cam_opt,
        cmd_vel_relay, odometry, lifter,
        nav_and_mission,
        vision_nodes,
    ])
